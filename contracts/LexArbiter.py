...
LEX_ARBITER_CONSTITUTION_V1 = """
LEX ARBITER — INDEPENDENT FORENSIC ADJUDICATOR, CONSTITUTION V1
...
or improve its requirements. Begin with the mandate, not the provider's narrative.
...
def _fetch_sources(manifest: dict) -> list[dict]:
    ...
    for item in raw_items:
        ...
        response = gl.nondet.web.get(url, headers={"User-Agent": "LexArbiter/1.0"})
...
def _build_prompt(policy: str, mandate: dict, manifest: dict, sources: list[dict]) -> str:
    output_schema = {
...
class LexArbiter(gl.Contract):
    owner: Address
    operator: Address
    cases: TreeMap[str, ArbiterCase]
    ...
    def submit_case(
        ...
        # Deterministic closure pre-extraction: pure Python primitives without self references
        m_manifest = manifest
        m_mandate = mandate
        m_policy = policy
        m_case_id = case_id
        ...
        def leader_fn():
            sources = _fetch_sources(m_manifest)
            prompt = _build_prompt(m_policy, m_mandate, m_manifest, sources)
            raw = gl.nondet.exec_prompt(prompt, response_format="json")
            judgment = _normalize_judgment(raw, m_mandate, sources)
            judgment["caseId"] = m_case_id
        ...
        self.cases[case_id] = ArbiterCase(
            case_id=case_id,
    ...
    def get_metrics(self) -> dict:
        return {
            ...
            "constitution_hash": _keccak_hex(LEX_ARBITER_CONSTITUTION_V1),
        }
